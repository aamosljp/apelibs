...
#ifndef APEBUILD_EXTERNAL_APEDSA
...
#else
#include "apedsa.h"
#endif /* APEBUILD_EXTERNAL_APEDSA */
#ifndef APEBUILD_INCLUDED
#define APEBUILD_INCLUDED

#define APEBUILD_VERSION_MAJOR 1
#define APEBUILD_VERSION_MINOR 0
...
#if defined(APEBUILD_WINDOWS)
#include <windows.h>
typedef PROCESS_INFORMATION ApeBuildProcHandle;
#define APEBUILD_INVALID_PROC_HANDLE (NULL)
#else
typedef pid_t ApeBuildProcHandle;
#define APEBUILD_INVALID_PROC_HANDLE (-1)
#endif
...
#define apebuild_cmd_append(cmd, arg) apedsa_da_push((cmd), (arg))
#define apebuild_cmd_append_many(cmd, ...)                                \
	do {                                                              \
		char *_args[] = { __VA_ARGS__ };                          \
		int _i = apedsa_da_count((cmd));                          \
		apedsa_da_addn((cmd), sizeof(_args) / sizeof(_args[0]));  \
		for (size_t _j = _i; _j < apedsa_da_count((cmd)); _j++) { \
			((cmd)[_j]) = _args[_j - _i];                     \
		}                                                         \
	} while (0)

#define apebuild_print(fmt, ...) fprintf(stderr, fmt, ##__VA_ARGS__)
#define apebuild_error(fmt, ...) apebuild_print("ERROR: (%s:%d) " fmt, __FILE__, __LINE__, ##__VA_ARGS__)

#if defined(__cplusplus)
extern "C" {
#endif

int apebuild_cmd_run_sync(ApeCmd cmd);
ApeBuildProcHandle apebuild_cmd_run_async(ApeCmd cmd);
int apebuild_cmd_poll(ApeBuildProcHandle handle);
int apebuild_cmd_wait(ApeBuildProcHandle handle);
int apebuild_cmds_run_seq(ApeCmd *cmds);
ApeBuildProcHandle *apebuild_cmds_run_parallel(ApeCmd *cmds);
...
#include "../apedsa/apedsa_api.h"


/* BEGIN cmd.c */

APEBUILD_DEF int apebuild_cmd_run_sync(ApeCmd cmd)
{
	ApeBuildProcHandle handle = apebuild_cmd_run_async(cmd);
	int status = apebuild_cmd_wait(handle);
	return status;
}

APEBUILD_PRIVATE void apebuild_cmd_render(ApeCmd cmd)
{
	apebuild_print("CMD: ");
	for (size_t i = 0; i < apedsa_da_count(cmd); i++) {
		apebuild_print("%s ", cmd[i]);
	}
	apebuild_print("\n");
}

APEBUILD_DEF ApeBuildProcHandle apebuild_cmd_run_async(ApeCmd cmd)
{
	apebuild_cmd_render(cmd);
#if defined(APEBUILD_WINDOWS)
	...
	if (!CreateProcessA(NULL, cmdline, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
		return APEBUILD_INVALID_PROC_HANDLE;
	}
	return pi;
#else
	apebuild_cmd_append(cmd, NULL);
	pid_t pid = fork();
	if (pid == 0) {
		execvp(cmd[0], cmd);
		exit(1);
...
APEBUILD_DEF int apebuild_cmd_poll(ApeBuildProcHandle handle)
{
#if defined(APEBUILD_WINDOWS)
	DWORD exit_code;
...
#else
	int status;
	waitpid(handle, &status, WNOHANG);
	return status;
#endif
}

APEBUILD_DEF int apebuild_cmd_wait(ApeBuildProcHandle handle)
{
#if defined(APEBUILD_WINDOWS)
...
APEBUILD_DEF int apebuild_cmds_run_seq(ApeCmd *cmds)
{
	int status = 0;
	for (size_t i = 0; i < apedsa_da_count(cmds); i++) {
		status = apebuild_cmd_run_sync(cmds[i]);
		if (status != 0) {
			return status;
		}
	}
...
APEBUILD_DEF ApeBuildProcHandle *apebuild_cmds_run_parallel(ApeCmd *cmds)
{
	ApeBuildProcHandle *handles = NULL;
	for (size_t i = 0; i < apedsa_da_count(cmds); i++) {
		apedsa_da_push(handles, apebuild_cmd_run_async(cmds[i]));
		if (apedsa_da_last(handles) == APEBUILD_INVALID_PROC_HANDLE) {
			apebuild_error("Failed to run command %s", cmds[i][0]);
			return NULL;
		}
	}
	return handles;
